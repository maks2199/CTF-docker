...
- Разработка и тестрирование, работа с ядром и драйверами, исследование уязвимостей
...
5. В каких случаях вообще контейнер могут запустить с --cap-add=SYS_ADMIN?

- Docker-in-Docker (DinD). GitLab CI runners, Jenkins agents часто требуют это для сборки образов.

6. А что это такое: --security-opt apparmor=unconfined?

- AppArmor — это Linux Security Module (LSM), система мандатного контроля доступа в ядре. Работает поверх обычных Unix permissions.

7. Что такое --security-opt seccomp=unconfined?

- Seccomp (Secure Computing) — фильтр системных вызовов в ядре Linux. Ограничивает какие syscalls процесс может вызывать.

- AppArmor: ЧТО процесс может делать (файлы, сеть, mount...)
  Seccomp: КАКИЕ syscalls процесс может вызывать (низкий уровень)

## TODO:

- [x] Проверить все практические задания
  - [x] Задание 1. Privileged container
  - [x] Задание 2. docker socket
  - [x] Задание 3. capabilities
- [x] Статья в ТГ
- [x] Best practicies
  - [x] Демонсрация: Unprivileged user
  - [x] Демонсрация: Cap_drop, cap_add
- [x] Рассмотреть способы скинирования
  - [x] docker scout
  - [x] trivy
  - [x] dockle
  - [x] docker bench security
- [x] Проверить примеры инцидентов, найти источники
- [x] Рассмотреть buildkit
- [x] Пройтись по всей презентации и задать себе вопросы
- [ ] Прогнать задачи
- [ ] Показать презентацию кому-то еще
- [x] Добавить автора и ссылку
- [ ] Поправить вёрстку
